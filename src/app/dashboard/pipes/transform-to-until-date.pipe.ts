...
export class TransformToUntilDatePipe implements PipeTransform {
  transform(dateStr: string): string {
    ...
    const diffTotalSecond: number = (new Date().getTime() - date.getTime()) / 1000;
    const daydiff: number = Math.floor(diffTotalSecond / 86400);
    ...
    if (daydiff === 0) {
      if (diffTotalSecond < 60) {
        untildDateResult = 'à l\'instant';
        ...
        untildDateResult = `il y a ${Math.floor(diffTotalSecond / 3600)} heures`;
      }
    } else if (daydiff === 1) {
      untildDateResult = 'hier';
    } else if (daydiff < 7) {
      untildDateResult = `il y a ${daydiff} jours`;
